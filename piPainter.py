...
mainMenu2ndLine = [
    lightMenu.strElem(["configure","loop","stop","halt"])];
    ...
    for b in GPIOButtons:
        ...
        if (b[0] is not prevIO) and input:
            ...
            if loop == 0:
                ...
                # save it to play it around
                if loopanim is 0 :
                    # save it to play it around
                    loopanim = b[1]
                else:
                    #turn off bar ans stop
                    filename = "%s/off.png"%pngDirName
                    offImg = lightImage.lightImage(filename,32)
                    offImg.paint_step()
                    loopanim = 0
